...
const CompletionandDelivery = () => {
  ...
  const fetchCompletionTerms = async () => {
    try {
      setTermsData((prev) => ({ ...prev, loading: true, error: null }));
      const response = await AdminApi.listCompletionTerms();
      ...
      if (response.status === 'Success' && response.data) {
        ...
        throw new Error(response.message || 'Failed to fetch completion terms');
      }
    } catch (error) {
      setTermsData({
        loading: false,
        error: error.message || 'Failed to fetch completion terms',
        data: [],
      });
    }
  };
  ...
  useEffect(() => {
    fetchCompletionTerms();
  }, []);

  const handleEdit = (term) => {
    setSelectedTerm(term);
    setEditTermText(term.delivery);
  ...
  const handleEditSubmit = async () => {
    if (!editTermText.trim()) {
      setNotification({
        isOpen: true,
        type: 'error',
        message: 'Please enter completion terms',
      });
      return;
    }
    ...
    try {
      const response = await AdminApi.editCompletionTerms(
        selectedTerm.id,
        editTermText
      );
      if (response.status === 'Success') {
        setNotification({
          isOpen: true,
          type: 'success',
          message: 'Completion terms updated successfully',
        });
        setIsEditModalOpen(false);
        fetchCompletionTerms(); // Refresh the list
      } else {
        throw new Error(
          response.message || 'Failed to update completion terms'
        );
      }
    } catch (error) {
      setNotification({
        isOpen: true,
        type: 'error',
        message: error.message || 'Failed to update completion terms',
      });
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    setIsSubmitting(true);
    try {
      const response = await AdminApi.deleteCompletionTerms(selectedTerm.id);
      if (response.status === 'Success') {
        setNotification({
          isOpen: true,
          type: 'success',
          message: 'Completion terms deleted successfully',
        });
        setIsDeleteModalOpen(false);
        fetchCompletionTerms(); // Refresh the list
      } else {
        throw new Error(
          response.message || 'Failed to delete completion terms'
        );
      }
    } catch (error) {
      setNotification({
        isOpen: true,
        type: 'error',
        message: error.message || 'Failed to delete completion terms',
      });
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-gray-900">
                Update Completion & Delivery Term
              </h2>
              <button
          ...
          >
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                Delete Completion & Delivery Term
              </h3>
...
export default CompletionandDelivery;
