...
const Paymentterms = () => {
  ...
  const fetchPaymentTerms = async () => {
    try {
      ...
      const response = await AdminApi.listPaymentTerms();
      ...
      setTermsData({
        loading: false,
        error: error.message || 'Failed to fetch payment terms',
        data: [],
      });
    }
  };
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await AdminApi.editPaymentTerms(
        selectedTerm.id,
        editTermText
      );
      if (response.status === 'Success') {
        ...
        throw new Error(response.message || 'Failed to update payment terms');
      }
    } catch (error) {
      setNotification({
        isOpen: true,
        type: 'error',
        message: error.message || 'Failed to update payment terms',
      });
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    setIsSubmitting(true);
    try {
      const response = await AdminApi.deletePaymentTerms(selectedTerm.id);
      if (response.status === 'Success') {
        setNotification({
          isOpen: true,
          type: 'success',
        ...
        throw new Error(response.message || 'Failed to delete payment terms');
      }
    } catch (error) {
      setNotification({
        isOpen: true,
