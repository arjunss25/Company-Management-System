...
const InvoicePending = () => {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const response = await axiosInstance.get('/list-invoice-pending/', {
        params,
      });
  ...
  // Update handleApplyDateFilters to use the new endpoint
  const handleApplyDateFilters = async (filters) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/filter-invoice-pending-date/${formatDateForApi(
          filters.dateFrom
  ...
  const handleEdit = (quotation) => {
    if (hasPermission(PERMISSIONS.EDIT_INVOICE_PENDING)) {
      navigate(`/edit-work-details/${quotation.id}`);
    }
  };

  const handleCancel = (quotation) => {
    if (hasPermission(PERMISSIONS.DELETE_INVOICE_PENDING)) {
  ...
  const handlePrintPDF = async (quotationId) => {
    if (hasPermission(PERMISSIONS.EXPORT_INVOICE_PENDING)) {
      try {
      ...
      <div className="flex-1 md:w-[calc(100%-300px)]">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight text-center">
            Documentation Status - Invoice Pending
          </h1>
          {/* Date Filter */}
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[300px] text-gray-500">
                  ...
                  <p className="text-xl font-medium">
                    No Invoice Pending Quotations
                  </p>
                  {dateFilters.dateFrom && dateFilters.dateTo ? (
                    <>
                      <p className="mt-2 text-gray-400">
                        No invoice pending quotations found for the selected
                        date range
                      </p>
                      <button
                        onClick={clearFilters}
                    ...
                    <p className="mt-2 text-gray-400">
                      No invoice pending quotations found in the system
                    </p>
                  )}
                </div>
              ) : (
                          ...
                          <div className="flex items-center space-x-4">
                            <motion.button
                              ...
                              data-tooltip-content={
                                !hasPermission(PERMISSIONS.EDIT_INVOICE_PENDING)
                                  ? "You don't have permission to edit invoice pending quotations"
                                  : ''
                              }
                              whileHover={
                                hasPermission(PERMISSIONS.EDIT_INVOICE_PENDING)
                              ...
                              <motion.button
                                ...
                                data-tooltip-content={
                                  !hasPermission(
                                    ...
                                    ? "You don't have permission to cancel invoice pending quotations"
                                    : ''
                                }
                                whileHover={
                                  hasPermission(
                            ...
                            <motion.button
                              ...
                              data-tooltip-content={
                                !hasPermission(
                                  PERMISSIONS.EXPORT_INVOICE_PENDING
                                )
                                  ? "You don't have permission to export invoice pending quotations"
