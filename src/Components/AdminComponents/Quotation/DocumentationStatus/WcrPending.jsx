...
const WcrPending = () => {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const response = await axiosInstance.get('/list-wcr-pending/', {
        params,
      });
  ...
  // Update the handleApplyDateFilters function
  const handleApplyDateFilters = async (filters) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/filter-wcr-pending-date/${formatDateForApi(
          filters.dateFrom
  ...
  const handleEdit = (quotation) => {
    if (hasPermission(PERMISSIONS.EDIT_WCR_PENDING)) {
  ...
  const handlePrintPDF = async (quotationId) => {
    if (hasPermission(PERMISSIONS.EXPORT_WCR_PENDING)) {
      try {
  ...
  const handleCancel = (quotation) => {
    if (hasPermission(PERMISSIONS.DELETE_WCR_PENDING)) {
      ...
      <div className="flex-1 md:w-[calc(100%-300px)] h-screen overflow-y-auto">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight text-center">
            Documentation Status - WCR Pending
          </h1>
          {/* Date Filter */}
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[300px] text-gray-500">
                  ...
                  <p className="text-xl font-medium">
                    No WCR Pending Quotations
                  </p>
                  {dateFilters.dateFrom && dateFilters.dateTo ? (
                    <>
                      <p className="mt-2 text-gray-400">
                        No WCR pending quotations found for the selected date
                        range
                      </p>
                      <button
                        onClick={clearFilters}
                    ...
                    <p className="mt-2 text-gray-400">
                      No WCR pending quotations found in the system
                    </p>
                  )}
                </div>
              ) : (
                          ...
                          <div className="flex items-center space-x-4">
                            <motion.button
                              ...
                              data-tooltip-content={
                                !hasPermission(PERMISSIONS.EDIT_WCR_PENDING)
                                  ? "You don't have permission to edit WCR pending quotations"
                                  : ''
                              }
                              whileHover={
                                hasPermission(PERMISSIONS.EDIT_WCR_PENDING)
                            ...
                            {!quotation.is_cancelled && (
                              <motion.button
                                data-tooltip-id="action-tooltip"
                                data-tooltip-content={
                                  !hasPermission(PERMISSIONS.DELETE_WCR_PENDING)
                                    ? "You don't have permission to cancel WCR pending quotations"
                                    : ''
                                }
                                whileHover={
                                  hasPermission(PERMISSIONS.DELETE_WCR_PENDING)
                            ...
                            <motion.button
                              data-tooltip-id="action-tooltip"
                              data-tooltip-content={
                                !hasPermission(PERMISSIONS.EXPORT_WCR_PENDING)
                                  ? "You don't have permission to export WCR pending quotations"
                              ...
                              disabled={
                                !hasPermission(
                                  PERMISSIONS.EXPORT_WCR_PENDING
                                ) || loadingPdfId === quotation.id
                              }
                              className={`p-2 rounded-lg transition-colors duration-300 ${
                                !hasPermission(PERMISSIONS.EXPORT_WCR_PENDING)
