...
const RetentionInvoiceSubmitted = () => {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        '/list-retention-invoice-submitted/',
        {
          params,
        }
      );
  ...
  const handleApplyDateFilters = async (filters) => {
    try {
      ...
      const response = await axiosInstance.get(
        `/filter-retention-invoice-submitted-date/${formatDateForApi(
          filters.dateFrom
  ...
  const handlePrintPDF = async (quotationId) => {
    if (hasPermission(PERMISSIONS.EXPORT_RETENTION_SUBMITTED)) {
      try {
  ...
  const handleCancel = (quotation) => {
    if (hasPermission(PERMISSIONS.DELETE_RETENTION_SUBMITTED)) {
  ...
  const handleEdit = (quotation) => {
    if (hasPermission(PERMISSIONS.EDIT_RETENTION_SUBMITTED)) {
      ...
      <div className="flex-1 md:w-[calc(100%-300px)] h-screen overflow-y-auto">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-gray-900 tracking-tight text-center mb-8">
            Documentation Status - Retention Submitted
          </h1>
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[300px] text-gray-500">
                  ...
                  <p className="text-xl font-medium">
                    No Retention Invoice Submitted Quotations
                  </p>
                  {dateFilters.dateFrom && dateFilters.dateTo ? (
                    <>
                      <p className="mt-2 text-gray-400">
                        No retention invoice submitted quotations found for the
                        selected date range
                      </p>
                      <button
                        onClick={clearFilters}
                    ...
                    <p className="mt-2 text-gray-400">
                      No retention invoice submitted quotations found in the
                      system
                    </p>
                  )}
                </div>
                          ...
                          <div className="flex items-center justify-center space-x-4">
                            <motion.button
                              ...
                              data-tooltip-content={
                                !hasPermission(
                                  PERMISSIONS.EDIT_RETENTION_SUBMITTED
                                )
                                  ? "You don't have permission to edit retention submitted quotations"
                                  : ''
                              }
                              whileHover={
                                hasPermission(
                            ...
                            {!quotation.is_cancelled && (
                              <motion.button
                                ...
                                data-tooltip-content={
                                  !hasPermission(
                                    PERMISSIONS.DELETE_RETENTION_SUBMITTED
                                  )
                                    ? "You don't have permission to cancel retention submitted quotations"
                                    : ''
                                }
                                whileHover={
                                  hasPermission(
                            ...
                            <motion.button
                              ...
                              data-tooltip-content={
                                !hasPermission(
                                  PERMISSIONS.EXPORT_RETENTION_SUBMITTED
                                )
                                  ? "You don't have permission to export retention submitted quotations"
...
export default RetentionInvoiceSubmitted;
